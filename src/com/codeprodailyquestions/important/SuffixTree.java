package com.codeprodailyquestions.important;
/*
...
class SuffixTree {
	...
	class Suffix {
		...
		/**
		 * Function Canonize() A suffix in the tree is denoted by a Suffix structure
		 * that denotes its last character. The canonical representation of a suffix for
		 * this algorithm requires that the origin_node by the closest node to the end
		 * of the tree. To force this to be true, we have to slide down every edge in
		 * our current path until we reach the final node
		 **/
	...
	class Edge {
		...
		/**
		 * function Insert () A given edge gets a copy of itself inserted into the table
		 * with this function. It uses a linear probe technique, which means in the case
		 * of a collision, we just step forward through the table until we find the
		 * first unused slot.
		 **/
		public void Insert() {
		...
		/**
		 * function SplitEdge () This function is called to split an edge at the point
		 * defined by the Suffix argument
		 **/
		public int SplitEdge(Suffix s) {
			Remove();
			Edge new_edge = new Edge(first_char_index, first_char_index + s.last_char_index - s.first_char_index,
					s.origin_node);
			new_edge.Insert();
		...
		/**
		 * function Remove () This function is called to remove an edge from hash table
		 **/
		public void Remove() {
			int i = Hash(start_node, T[first_char_index]);
	...
	/**
	 * Function Hash() - edges are inserted into the hash table using this hashing
	 * function
	 **/
	...
	/**
	 * Function AddPrefix() - called repetitively, once for each of the prefixes of
	 * the input string
	 **/
	...
	public void dump_edges(int current_n) {
		...
		for (int j = 0; j < HASH_TABLE_SIZE; j++) {
			...
			if (s.start_node == -1)
				continue;
			System.out.printf("%5d %5d %3d %5d %6d   ", s.start_node, s.end_node, Nodes[s.end_node].suffix_node,
